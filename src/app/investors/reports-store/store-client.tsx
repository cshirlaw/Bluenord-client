...
import TypeDropdown from "@/components/TypeDropdown"; // reuse (presentation/report/annual/all)
...
export default function ReportsStoreClient() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const arr: Item[] = (Array.isArray(data?.items) ? data.items : []).map((x: Item) => ({
          ...x, kind: detectKind(x)
        }));
    ...
    <div className="space-y-6">
      ...
      {error && (
        <div className="rounded-xl border border-red-300 bg-red-50 p-4 text-sm text-red-700">
          ...
          <div>{error}</div>
          <div className="mt-2 opacity-70">Tried: /api/reports</div>
        </div>
      )}
