...
export default async function TyraPage() {
  ...
  const summary = tyra?.summary ?? "Overview, media, and key milestones for the Tyra redevelopment.";
  ...
  const photos = (tyra?.photos?.length ? tyra.photos : [{ src: image, alt }]).map(p => ({
